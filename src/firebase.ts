...
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null) {
  ...
  if (errInfo.error && errInfo.error.includes("Missing or insufficient permissions")) {
    alert(`Data update failed for ${path}.\nYou might not be properly authenticated as the administrator. Please log out and log in again using Admin Login.`);
  } else {
    alert(`Database Error (${operationType} on ${path}): ` + errInfo.error);
  }
}
